...
do
    ...
    python oncotypedx_MLC_aux_learn_prototype.py --mccv=$i --main_task_file="$main_task_file"
done

echo "All iterations completed."

# Concatenate all files into one, including the header from the first file
echo "AUROC,AUPR" > "./output/combined_${base_name}.csv"
for i in {0..29}
do
    # Skip header (first line) for all files except the first one
    if [ $i -eq 0 ]; then
        cat "./output/MCCV_oncotype_aux_learn_${base_name}_$i.csv" >> "./output/combined_${base_name}.csv"
    else
        tail -n +2 "./output/MCCV_oncotype_aux_learn_${base_name}_$i.csv" >> "./output/combined_${base_name}.csv"
    fi
done
