...
class BreastCancerDataset(Dataset):
    ...
    def preprocess_data(self, file_paths, top_genes):
        ...
        for file_idx, file_path in enumerate(file_paths):
            ...
            # Apply SMOTE to the raw features and labels
            smote = SMOTE()
            smote_features, smote_labels = smote.fit_resample(preprocessed_df.drop('Sample', axis=1), actual_labels)
            
            # Create a new DataFrame with SMOTE output
            smote_df = pd.DataFrame(smote_features, columns=new_column_names)
            smote_df['Sample'] = np.repeat(df['Sample'].values, np.ceil(len(smote_labels) / len(actual_labels)).astype(int))[:len(smote_labels)]
            
            for i, row in smote_df.iterrows():
                ...
                if sample_id not in sample_id_to_index:
                    ...
                    new_label_row[file_idx] = smote_labels[i]
                    # new_label_row[file_idx] = actual_labels[i]
                    all_labels.append(new_label_row)
                    ...
                    all_labels[idx][file_idx] = smote_labels[i]
